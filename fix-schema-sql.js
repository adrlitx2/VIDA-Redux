// Fix schema using the same approach that worked to create tables originally
...
async function fixSchema() {
  ...
  try {
    ...
    if (dropError) {
      ...
      // Try using the same approach that worked for creating tables
      const { error: altDropError } = await supabase.rpc('exec_sql', {
        sql: 'ALTER TABLE co_stream_invitations DROP CONSTRAINT co_stream_invitations_session_id_fkey;'
      });
      
      if (altDropError) {
        console.error('❌ Alternative approach also failed:', altDropError);
        return;
      } else {
