...
export class ImageCharacterAnalyzer {
  ...
  async analyzeCharacter(imageBuffer: Buffer): Promise<CharacterAnalysis> {
    ...
    try {
      ...
      const negativePrompt = this.generateEnhancedNegativePrompt(analysis);
  ...
  private analyzePose(samples: any, bodyParts: any) {
    ...
    // Detect asymmetrical poses that need correction
    const asymmetricalPose = this.detectAsymmetricalPose(samples);
    
    let armPosition: 'up' | 'down' | 'crossed' | 'extended' | 'mixed' | 'unknown' = 'unknown';
    
    if (leftArmPresent && rightArmPresent) {
      if (asymmetricalPose.oneArmRaised) {
        armPosition = 'mixed'; // One arm up, one down - needs correction
      } else {
        armPosition = 'extended'; // Both arms in similar position
      }
    ...
    return {
      ...
      needsTPose: true, // Always force T-pose for consistency
      asymmetricalPose
    };
  }
  
  /**
   * Detect asymmetrical poses that cause generation issues
   */
  private detectAsymmetricalPose(samples: any) {
    // Analyze arm positions by looking at different height regions
    const upperRegion = samples.torso.slice(0, Math.floor(samples.torso.length * 0.3));
    const middleRegion = samples.torso.slice(Math.floor(samples.torso.length * 0.3), Math.floor(samples.torso.length * 0.7));
    
    // Check for one arm raised vs one arm down pattern
    const leftArmActivity = samples.leftArm.length;
    const rightArmActivity = samples.rightArm.length;
    
    const asymmetryRatio = leftArmActivity > 0 && rightArmActivity > 0 
      ? Math.abs(leftArmActivity - rightArmActivity) / Math.max(leftArmActivity, rightArmActivity)
      : 0;
    
    return {
      oneArmRaised: asymmetryRatio > 0.5, // High asymmetry indicates one arm up
      asymmetryLevel: asymmetryRatio,
      needsCorrection: asymmetryRatio > 0.3 // Correction needed if significantly asymmetrical
    };
  }
  ...
  private generateAIEnhancedTPosePrompt(analysis: CharacterAnalysis): string {
    ...
    // Critical T-pose requirements - FORCE symmetrical pose
    components.push('MUST BE IN PERFECT T-POSE STANCE');
    components.push('BOTH arms extended horizontally at exact same height');
    components.push('arms perfectly parallel to ground');
    components.push('left arm and right arm at identical shoulder height');
    components.push('NO asymmetrical arm positioning');
    components.push('NO one arm raised higher than the other');
    
    // Asymmetrical pose correction - COMPLETE OVERRIDE
    if (analysis.pose?.asymmetricalPose?.oneArmRaised) {
      components.push('COMPLETELY OVERRIDE original arm positioning');
      components.push('REPLACE any raised arm with horizontal T-pose');
      components.push('DISCARD source pose completely');
      components.push('NO extended arm gestures');
      components.push('NO arm pointing');
      components.push('NO reaching poses');
      components.push('CORRECT any asymmetrical arm positioning');
      components.push('FORCE both arms to same horizontal level');
      components.push('IGNORE original arm positioning');
      components.push('OVERRIDE any raised arm with proper T-pose');
    }
    
    // MediaPipe pose insights with T-pose override
    if (analysis.mediaPipeAnalysis) {
      ...
      // Override any detected pose to ensure T-pose
      components.push('OVERRIDE detected pose with T-pose');
    }
    
    // Enhanced T-pose requirements with arm limitation
    components.push('EXACTLY TWO ARMS ONLY');
    components.push('arms extended horizontally to the sides at shoulder height');
    components.push('legs straight and slightly apart');
    components.push('facing forward');
    components.push('symmetrical body positioning');
    components.push('ONLY TWO ARMS TOTAL');
    components.push('NO EXTRA ARMS');
    components.push('NO ADDITIONAL LIMBS');
    
    // AI-informed anatomical completeness
    components.push('anatomically correct proportions');
    components.push('detailed character design');
    components.push('complete left arm, right arm, left leg, right leg, feet');
    components.push('equal length arms');
    components.push('proportional limb sizing');
    ...
    // Final requirements
    components.push('full body visible');
    components.push('neutral T-pose stance');
    components.push('standing upright');
  ...
  /**
   * Generate enhanced negative prompt with AI insights
   */
  private generateEnhancedNegativePrompt(analysis: Omit<CharacterAnalysis, 'tPosePrompt' | 'negativePrompt'>): string {
    const negatives = [
      ...
      // Asymmetrical pose negatives - CRITICAL for fixing the issue
      'asymmetrical arms', 'uneven arms', 'one arm raised', 'one arm up', 'one arm down',
      'different arm heights', 'arm length differences', 'unequal arm positioning',
      'raised arm', 'lowered arm', 'arm asymmetry', 'arm imbalance',
      'extended arm', 'reaching arm', 'arm gesture', 'arm pointing',
      
      // Arm length and proportion negatives
      'extra long arms', 'oversized arms', 'stretched arms', 'elongated limbs',
      'disproportionate arms', 'wrong arm proportions', 'arm stretching',
      'arm distortion', 'arm deformation', 'unnatural arm length',
      
      // Extra arm and limb multiplication negatives - CRITICAL
      'extra arms', 'multiple arms', 'additional arms', 'too many arms',
      'duplicate arms', 'arm duplication', 'arm multiplication', 'three arms',
      'four arms', 'more than two arms', 'arm extending from hand',
      'arm growing from hand', 'hand with attached arm', 'arm sprouting from palm',
      'extra appendages', 'additional limbs', 'limb duplication', 'appendage multiplication',
      
      // Original pose retention negatives - NEW
      'keep original arm position', 'maintain original pose', 'preserve arm gesture',
      'retain raised arm', 'keep extended arm', 'maintain arm asymmetry',
      'original arm positioning', 'source pose retention', 'pose preservation',
      ...
      'rough textures', 'poor geometry', 'pixelated', 'low quality'
    ];
    
    // CLIP analysis insights
    if (analysis.clipAnalysis) {
      // Add negatives based on what CLIP detected to avoid
      if (analysis.clipAnalysis.artStyleConfidence < 0.7) {
        negatives.push('inconsistent art style', 'mixed styles');
      }
    }
    
    // MediaPipe pose insights
    if (analysis.mediaPipeAnalysis) {
      // Add pose-specific negatives based on detected pose
      if (analysis.mediaPipeAnalysis.estimatedPose === 'side') {
        negatives.push('side profile', 'lateral view');
      }
      if (analysis.mediaPipeAnalysis.poseScore < 0.5) {
        negatives.push('unclear pose', 'ambiguous stance');
      }
    }
    ...
    // Add anatomy-specific negatives based on confidence
    if (analysis.anatomyConfidence < 0.7) {
      negatives.push('unclear anatomy', 'poorly defined limbs', 'ambiguous body structure');
    }
