    ...
    try {
      ...
      while (attempts < maxAttempts) {
        ...
        try {
          ...
          // ✨ GROK 4.0 ENHANCEMENT: Incorporate pose normalization prompts
          let enhancedTPrompt = characterAnalysis.tPosePrompt;
          let enhancedNegativePrompt = characterAnalysis.negativePrompt;
          
          if (options.poseNormalized && options.enhancedPrompts) {
            console.log('🎯 Applying pose normalization enhancements...');
            enhancedTPrompt = `${characterAnalysis.tPosePrompt} ${options.enhancedPrompts.join(' ')}`;
            enhancedNegativePrompt = `${characterAnalysis.negativePrompt} extra arms, multiple arms, arm extending from hand, arm growing from hand, hand with attached arm, arm sprouting from palm, asymmetrical pose, raised arm, bent arm, non-T-pose stance`;
            console.log('✅ Enhanced prompts applied for pose normalization');
          }
