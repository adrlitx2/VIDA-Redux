...
export async function registerRoutes(app: Express, wsInstance?: any, io?: any): Promise<Server> {
  ...
  app.post("/api/avatars/2d-to-3d", async (req: any, res, next) => {
    ...
    try {
      ...
      // ✨ GROK 4.0 ENHANCEMENT: Apply pose normalization before 3D conversion
      console.log('🔄 Starting pose normalization pipeline...');
      let normalizedImagePath = req.file.path;
      let poseNormalizationResult;
      
      try {
        poseNormalizationResult = await poseNormalizationService.normalizePose(req.file.path);
        normalizedImagePath = poseNormalizationResult.normalizedImagePath;
        
        console.log('✅ Pose normalization completed:', {
          hasAsymmetricalPose: poseNormalizationResult.originalPose.hasAsymmetricalPose,
          asymmetryRatio: poseNormalizationResult.originalPose.asymmetryRatio,
          normalizationApplied: poseNormalizationResult.normalizationApplied.poseCorrection,
          processingSteps: poseNormalizationResult.processingSteps.length
        });
      } catch (poseError) {
        console.warn('⚠️ Pose normalization failed, using original image:', poseError);
        poseNormalizationResult = null;
      }
      
      // Read the normalized image (or original if normalization failed)
      const normalizedImageBuffer = fs.readFileSync(normalizedImagePath);
      console.log('📄 Using image buffer size:', normalizedImageBuffer.length, 'bytes');
      
      // Clean up the temporary files
      try {
        fs.unlinkSync(req.file.path);
        if (normalizedImagePath !== req.file.path) {
      ...
      // Use Meshy AI service for 2D to 3D conversion with normalized image
      const { meshy2DTo3DConverter } = await import('./services/meshy-2d-to-3d-converter.js');
      
      // Get enhanced prompts from pose normalization result
      const enhancedPrompts = poseNormalizationResult ? 
        poseNormalizationService.getPreprocessingRecommendations(poseNormalizationResult).enhancedPrompts : [];
      
      const conversionResult = await meshy2DTo3DConverter.convertImageTo3D(normalizedImageBuffer, {
        userPlan,
        ...
        poseNormalized: poseNormalizationResult?.normalizationApplied.poseCorrection || false,
      ...
      const avatar = {
        ...
        metadata: {
          ...
          poseNormalization: poseNormalizationResult ? {
            hasAsymmetricalPose: poseNormalizationResult.originalPose.hasAsymmetricalPose,
            asymmetryRatio: poseNormalizationResult.originalPose.asymmetryRatio,
            normalizationApplied: poseNormalizationResult.normalizationApplied.poseCorrection,
            processingSteps: poseNormalizationResult.processingSteps.length,
            detectedIssues: poseNormalizationResult.originalPose.detectedIssues
          } : null
        }
      };
