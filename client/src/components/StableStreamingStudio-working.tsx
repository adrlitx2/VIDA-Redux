...
export default function StableStreamingStudio() {
  ...
  const [selectedVirtualBg, setSelectedVirtualBg] = useState('83');
  ...
  const handleAvatarSelect = (avatar: any, type: 'user' | 'preset') => {
    setSelectedAvatar(avatar);
    setSelectedAvatarType(type);
    setAvatarEnabled(true); // Auto-enable avatar when selected
  };
  ...
  useEffect(() => {
    if (avatars && avatars.length > 0 && !selectedAvatar) {
      console.log('🎭 Auto-selecting first avatar:', avatars[0]);
      setSelectedAvatar(avatars[0]);
      setSelectedAvatarType('user');
    }
  }, [avatars, selectedAvatar]);

  // Debug avatar state
  useEffect(() => {
    console.log('🎭 Avatar state:', { avatarEnabled, selectedAvatar: selectedAvatar?.name, selectedAvatarType });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800">
      ...
      {/* Hidden 3D Avatar Viewer for Stream Canvas */}
      {avatarEnabled && selectedAvatar && (
        <div id="avatar-container" className="fixed top-0 left-0 w-64 h-64 pointer-events-none opacity-100 z-[1000] border-2 border-red-500 bg-black/20">
          <div className="text-white text-sm p-2 bg-black/80">
          ...
          <ThreeModelViewer
            ...
            bodyTracking={true}
            avatarType="fullbody"
            isRigged={selectedAvatar.isRigged}
            cameraStream={cameraStream}
